...
import ReactModal from "react-modal"; //모달 사용 필수 조건
...
function AddPostModal({isOpen, onRequestClose, layoutRef}) {
    ...
    const handleImageDeleteOnClick = (index) => {
        const deletedImages = uploadImages.filter((img, imgIndex) => index !== imgIndex)
                                                                // 조건이 true여야만 새로운 배열에 담는다
    ...
    const handlePostSubmitOnClick = async() => {
        ...
        for (let img of uploadImages) { //사진 파일 객체를 넘겨야 됨
